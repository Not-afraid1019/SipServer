...
RequestsHandler::RequestsHandler(std::string serverIp, int serverPort, RequestsHandler::OnHandledEvent onHandledEvent)
...
void RequestsHandler::initHandles() {
...
std::optional<std::shared_ptr<Session>> RequestsHandler::getSession(const std::string &callID) {
    auto sessionId = _sessions.find(callID);
    if (sessionId != _sessions.end()) {
...
void RequestsHandler::endCall(const std::string &callID, const std::string &srcNumber, const std::string &destNumber,
